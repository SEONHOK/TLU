...
for j=1:length(d_list)
    ...
    for t=1:total_iteration
        ...
        for k=1:K
%             w=[w  w_tmp];
            w_tmp=w_tmp-eta*leakyReLU_gradient(w0,w_tmp,n,X_sample,a);    %gradient descent step
            loss=[loss 1/(2*n)*sum((leakyReLU(w_tmp,X_sample,a)-leakyReLU(w0,X_sample,a)).^2)];
        end   
